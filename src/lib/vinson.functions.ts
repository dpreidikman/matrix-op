...
// Cron endpoint that syncs yesterday for every store. Exposed via server fn
// so the client never imports the admin client transitively.
export const cronSyncYesterday = createServerFn({ method: "POST" }).handler(
  async (): Promise<{
